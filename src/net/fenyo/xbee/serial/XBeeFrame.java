...
public class XBeeFrame extends XBeeData {
	...
	public String getAddress() throws UnsupportedEncodingException {
		if (api_id != 0x80)	return null;
		String address = "";
		for (final byte b : ArrayUtils.subarray(content, 0, 8)) address += ((b & 0xff) < 16 ? "0" : "") + Integer.toHexString(b & 0xff);
		return address;
	}
	...
	public String getReceivedData() throws UnsupportedEncodingException {
		if (api_id != 0x80)	return null;
		String msg = "";
		for (final byte b : ArrayUtils.subarray(content, 10, content.length)) msg += new String(new byte [] { b }, "ISO8859-1");
		return msg;
	}
	...
	public byte [] getRawContent() throws UnsupportedEncodingException {
		...
		for (final byte b : raw_content)
			...
			if (Arrays.binarySearch(new byte [] { 0x11, 0x13, 0x7d, 0x7e}, b) < 0) escaped_raw_content.add(b);
			else {
	...
	public String toString() {
		...
		for (final byte b : content) str += "       byte: " + Integer.toHexString(b & 0xff) + "h" + " " + (b & 0xff) + "d ('" + (char) b + "')\n";
		return str;
	}
