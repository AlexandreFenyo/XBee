	...
	public void run() {
		try {
			...
			while (true) {
				...
				if (xbee_data != null) {
					if (xbee_data.waitBeforeWrite() != 0) Thread.sleep(xbee_data.waitBeforeWrite());

					log.debug("write " + xbee_data.getRawContent().length + " bytes"); 
					for (int i = 0; i < xbee_data.getRawContent().length; i++)
						log.debug("write byte: " + Integer.toHexString(0xff & xbee_data.getRawContent()[i]) + " - char='" + new String(new byte [] { xbee_data.getRawContent()[i] }, "ASCII") + "'");
					
					stream.write(xbee_data.getRawContent());
					if (xbee_data.waitAfterWrite() != 0) Thread.sleep(xbee_data.waitAfterWrite());

				} else log.warn("wake-up for data but data is null");
			}
